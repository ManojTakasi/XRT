...
std::pair<size_t, size_t> gmio_api::enqueueBD(XAie_MemInst *memInst, uint64_t offset, size_t size)
{
    ...
    /* Runtime.io_uring: batch-submit early async ops, run sync desc setup in between,
     * then submit remaining async ops and wait once (review feedback: per-op submit+wait
     * matches sync ioctl cost; batching exposes io_uring value). */
    if (config->get_io_uring()) {
      XAie_AsyncRes ares[3]{};
      int n = 0;
      int r = XAie_DmaSetAddrOffsetLenAsync(&shimDmaInst, memInst, offset, static_cast<u32>(size), &ares[0]);
      if (r == 0)
        throw xrt_core::error(ares[0].res ? ares[0].res : -EIO,
            "XAie_DmaSetAddrOffsetLenAsync submit failed");
      n += r;
      ...
      r = XAie_DmaWriteBdAsync(config->get_dev(), &shimDmaInst, gmioTileLoc, bdNumber, &ares[1]);
      if (r == 0) {
        aie_async_wait_nr_throw(config->get_dev(), n);
        throw xrt_core::error(ares[1].res ? ares[1].res : -EIO, "XAie_DmaWriteBdAsync submit failed");
      }
      n += r;

      r = XAie_DmaChannelPushBdToQueueAsync(config->get_dev(), gmioTileLoc, pGMIOConfig->channelNum,
          (pGMIOConfig->type == gmio_config::gm2aie ? DMA_MM2S : DMA_S2MM), bdNumber, &ares[2]);
      if (r == 0) {
        aie_async_wait_nr_throw(config->get_dev(), n);
        throw xrt_core::error(ares[2].res ? ares[2].res : -EIO, "XAie_DmaChannelPushBdToQueueAsync submit failed");
      }
      n += r;
