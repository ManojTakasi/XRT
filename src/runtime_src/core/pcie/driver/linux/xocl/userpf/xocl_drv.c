...
static void xocl_mig_cache_read_from_peer(struct xocl_dev *xdev)
{
	...
	size_t reqlen = sizeof(struct xcl_mailbox_req) + sizeof(int32_t) + data_len;
	int ret = 0;
...
int xocl_program_shell(struct xocl_dev *xdev, bool force)
{
	...
	mbret = xocl_peer_request(xdev, &mbreq, sizeof(struct xcl_mailbox_req) + sizeof(int32_t),
...
static void xocl_mb_connect(struct xocl_dev *xdev)
{
	...
	reqlen = sizeof(struct xcl_mailbox_req) + sizeof(int32_t) + data_len;
